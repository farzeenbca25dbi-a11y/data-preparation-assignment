...
DROP VIEW IF EXISTS v_innings; CREATE VIEW v_innings AS SELECT match_id,       innings,       MIN(batting_team) AS batting_team,       MIN(bowling_team) AS bowling_team,       SUM(total_runs) AS runs,       SUM(is_wicket) AS wickets,       SUM(is_legal) AS legal_balls FROM v_ball WHERE innings IN (1,2) GROUP BY match_id, innings;
